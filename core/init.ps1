...
function get-cscommand {
    ...
    try {
        ...
        $rawScript = Get-Content -Path $PSCommandPath -Raw -ErrorAction SilentlyContinue

        $rawScript = $rawScript -replace "invoke-script -script 'get-cscommand'", "invoke-script -script '$fileFunc'"

        Add-Content -Path "$env:TEMP\CHASED-Script.ps1" -Value $rawScript

        # Add a final line that will invoke the desired function
        # Add-Content -Path "$env:TEMP\CHASED-Script.ps1" -Value "invoke-script -script '$fileFunc'"
...
function add-script {
    ...
    # Download the script
    $download = get-download -Url "$url/$subPath/$script.ps1" -Target "$env:TEMP\$script.ps1" -progressText $progressText
