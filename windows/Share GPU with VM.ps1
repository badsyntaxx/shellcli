...
function copyHostGPUDriversToVM {
    ...
    if (-not $guestService.Enabled) {
        writeText -type "notice" -text "Guest Service Interface is not enabled. Attempting to enable..."
        Enable-VMIntegrationService -VMName $vmName -Name "Guest Service Interface"
        writeText -type "notice" -text "Waiting for service to initialize..."
    ...
    writeText -type "notice" -text "VM and Integration Services verified. Proceeding with file copy..."
    ...
    if ($sourceFolder) {
        writeText -type "notice" -text "Found folder: $($sourceFolder.Name)"
        writeText -type "notice" -text "Full path: $($sourceFolder.FullName)"
    
        # Create copy command
        writeText -type "notice" -text "Will copy this folder to VM"
    } else {
    ...
    if ($sourceFolder) {
        writeText -type "notice" -text "Copying driver folder $($sourceFolder.Name)..."
    ...
    foreach ($file in $sourceFiles) {
        try {
            writeText -type "notice" -text "Copying $($file.Name)..."
            Copy-VMFile -VMName $vmName `
                -SourcePath $file.FullName `
    ...
    writeText -type "success" -text "Files copied to the VM at $tempVMPath. Now run the 'install host gpu drivers' command on the VM."
...
function installHostGPUDriversOnVM {
    ...
    writeText -type "success" -text "File copying completed!"
}
